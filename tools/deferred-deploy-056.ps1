# Deferred deploy for QuriousCraftingRelics 0.5.6: waits for the game to
...
foreach ($t in $targets) {
    ...
    if ($h -ne $hash -or $v -ne "0.5.6") { $ok = $false; Write-Output "MISMATCH $t dll=$($h -eq $hash) version=$v" }
}
if ($ok) { Write-Output "DEFERRED DEPLOY OK: 0.5.6, all targets hash+version verified" } else { Write-Output "DEFERRED DEPLOY FAILED" }
