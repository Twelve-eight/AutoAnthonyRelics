...
## 2026-09-23 设置页前端重写契约

用户本轮追加要求: 重写怪异炼化遗物 mod 的设置页前端, 与工作区审查并行. 本节只改变前端交互, 不更换 mod id, 配置键, 默认值, 生成算法, 稀有度规则或存档身份.

### 信息结构与视觉

- 保留原版设置中的独立入口及 NSubmenuStack/NBackButton 协议. 使用 Godot 原生 UI, 不引入浏览器, 图片生成资产或新依赖.
- 将长页面拆为基础规则与词条编辑两个明确页面, 统一标题, 使用说明与保存状态. 移除重复的预算编辑器标题以及通用设置与编辑器的重复价格列表.
- 基础规则只呈现有效的总开关, 额外池开关, 三档点数预算与负面概率. Legacy ChaosRelicMultiplier 仍保留兼容键但不再作为可调有效规则展示.
- 词条编辑支持文本搜索与分类筛选, 保留效果模板正文, 原版遗物参考及悬浮说明. 每行集中显示数值范围与每点花费/返还, 不让用户往返两段长列表寻找同一词条.
- 色彩采用深灰底, 米白主文本与克制的金色强调. 使用有层级的标题, 紧凑数值控件与分隔, 不增加纯装饰或占位数据. 错误状态可区分, 焦点可见.
- 1280x720 与 1920x1080 均为布局检查目标. 内容宽度随可用空间收缩, 不再强制每层 800 像素最小宽度. 不使用横向滚动承载关键操作.

### 编辑和持久化不变量

- 只读写 ModConfigRegistry 注册的实例及其现有静态属性, 不创建第二份配置. UI 构造与刷新不得保存或修改数值.
- 数值输入沿用对应 ConfigSlider 上下界/步长, 不抄写过期注释中的默认值. Min/Max 输入始终满足 low <= high, 大范围输入不能被 UI 默默截短.
- 预算编辑器明确使用 ChaosTemplates.Spec + QuriousCraftingRelicsConfig.ApplyUserBounds 获取实时编辑范围, 不使用局内冻结的 SpecOf/Effective. 调整 UI 只改变后续生成配置, 不修改正在使用的生成快照.
- UniqueOnly 直接使用生成器公开的只读集合来标识不参与生成的词条, 不复制三项常量清单. 对这类行明确标记并禁用无效编辑. 额外池关闭时仍能查看和预先调整对应设置, 显示当前未启用的说明.
- 原有 Changed 事件, 五秒 debounce, 隐藏页面和退出树立即保存均保留. 失败必须显示失败而不是已保存. 重新打开缓存页面刷新实时值, 事件订阅可回收, 不在输入刷新时递归写配置.
- 搜索和筛选不重建全部控件, 不变更数值, 不影响保存. 悬浮提示与页面退出清理遵循已有 NHoverTipSet 协议.
- 新增界面文案使用 eng/zhs 两份 settings_ui 资源, 复用既有术语并核对 G:\omp works\docs\terminology-glossary.md. 不删除旧配置本地化兼容键.

### 本轮验证边界

先保留旧代码结构证据, 实现者与监督者同批执行. 主会话集中进行语言检查, 本地化键/绑定覆盖检查与隔离 Release 构建, 显式禁用 CopyToModsFolderOnBuild. 未单独取得运行授权前不启动游戏, 不改共享配置或 Steam 安装; 截图, 键盘/手柄导航与真实保存重开必须标为尚待实机验收.
