## 2026-09-22 修复契约: 保存定义, 跨进程恢复

本次仅修复怪异炼化遗物主体. 新存档通过现有 BaseLib 扩展保存通道同时持久化身份, 冻结配置和 60 个完整生成定义; 读档不能根据当前偏好重新定义已持有遗物. 保存 payload 使用明确版本, 绑定身份与 seed, 拒绝截断, 超限, 未知格式和不兼容语义, 验证完成后一次性发布 active 上下文.

现有 identity key 保持兼容. 旧 token-only 存档缺少历史冻结值, 不承诺凭哈希找回它们: 对可验证重建和不能恢复分别报告, 不把已知漂移当成功; 无 token 旧档继续使用确定性身份, 不加随机后缀. 存储注册失败必须阻止不持久新局继续初始化. 新局 RNG/预算/采样顺序保持当前行为; 新存档直接恢复保存的定义而不重抽.

离开对局清空 active snapshot 和定义, 保留有界 ledger 作缓存. 外部 seed/菜单预览使用 live 配置, 活跃局沿用冻结输入和零分配查询. 模型保存, 进程退出重启, A/B/A, 缓存淘汰, 配置变更, 损坏 payload 与注册失败由主会话在 G: 隔离验收. 未经实际运行不声称游戏/联机验收通过.

详细计划: G:/omp works/docs/reviews/2026-09-22-fixes/PLAN.md. 本节优先于下文历史身份阶段允许按当前配置重生成的旧说明.
