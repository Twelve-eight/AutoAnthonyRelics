...
**跨进程验证通过 (2026-09-17, 同一副本)**: 结束实例 A 进程 -> 确认存档仍含 token -> 重新启动 ->
加载该存档. 新进程日志 `run identity minted` 计数 **0**, 而出现
`run identity qcr1:6388dd195dcc4585:..:MU25VDXKHR93 adopted from the save (seed MU25VDXKHR93)`.
即身份完全来自存档文件, 而非重新铸造 -- 这正是验收要求的"完全退出进程后继续 A".

**仍未证实**: 存档 A -> 存档 B -> 回存档 A 的跨存档路径(需要两次不同配置的对局)未实机跑过;
该条仍只有探针证据.
...
### G 第二部分: 拾起时附魔词条 (形态 B, 用户拍板两种形态并存)

- 新增 3 个额外池模板: X_PICKUP_SHARP / X_PICKUP_NIMBLE / X_PICKUP_IMBUED
  (Min/Max = 等级增量 1-2, Cost 6/6/5, 配置键齐全, 预算编辑器自动出现)。
- 执行: AfterObtained → ApplyPickupEnchant<T>: 牌组(主牌堆)中符合条件的卡
  (同附魔可叠 / 空位可附 / 异附魔排除) 用 Rng.UpFront 选一张,
  EnchantWithStacking 附加 {N} 级。RNG 通道 UpFront 为运行级种子通道,
...
## 2026-09-13 紧急修复 II: 商店只有头环 (袋空) — 保留契约重构为责任域剥离

- **现象**: 怪异炼化开 + 东尼遗物关 → 商店只有头环 (引擎 fallback), 无 mod 遗物。
- **根因** (日志实证): 东尼遗物补丁在 enabled=False/replaceVanilla=False 下按其全局
  策略剥离了全部 476 个 (含怪异炼化的 60 个开启遗物) → 袋空。两个 mod 各自
  RemoveAll 的叠加是**顺序依赖**的——这是架构错误, 不是谓词写错。
- **重构**: 责任域剥离契约。每个补丁只剥离自己负责的:
  1. 自家槽位, 当自身开关关闭 (无效果占位);
  2. 原版遗物, 当自身处于"替换开启"状态。
  他家遗物**永不触碰**——由各自的 IsAllowed + 引擎原生
  RemoveDisallowedRelicsFromDeques 强制 (补丁末尾调用)。
  四种开关组合全部顺序无关: 双开=混合池 / 仅怪异=怪异 only / 仅东尼=东尼 only /
  双关=原版。KeepModdedRelics 配置项被此契约取代, 已从配置与 loc 移除。
- 验证: 构建 0 错误, 已部署实机+staging。四种组合待用户真机验证。

## 2026-09-13: 手牌效果玩家自选 (用户报告: 「为手牌添加关键词的效果没有自选,固定第一张牌」)

- **根因**: TakeEligible 按手牌顺序取前 N 张 —— 无玩家选择。
- **修复**: 四类手牌效果改为引擎原生选牌 UI (CardSelectCmd.FromHand):
  保留/奇巧/虚无 (每回合开始, 至多 N 张) + 战斗开始附魔 (至多 N 张)。
